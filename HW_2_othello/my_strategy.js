    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            ...
            for (const [dx, dy] of directions) {
                ...
                while (isOnBoard(nx, ny) && boardState[nx][ny] === opponent) {
                    hasOpponent = true;
                    nx += dx;
                    ny += dy;
                }
...
function evaluateBoard(boardState, player) {
    ...
    // 게임 단계 결정 (보드의 비율로)
    const totalPieces = boardState.flat().filter((cell) => cell !== 0).length;
    ...
    const opponentMobility = getValidMovesForBoard(boardState, opponent).length;
...
function isStablePiece(boardState, row, col, stableBoard) {
    ...
    for (const axis of axes) {
        ...
        for (const [dx, dy] of axis) {
            ...
            if (
                !isOnBoard(nx, ny) ||
                (isOnBoard(nx - dx, ny - dy) && stableBoard[nx - dx][ny - dy])
            ) {
                stable = true;
                break;
            }
...
function minimax(boardState, depth, alpha, beta, isMaximizing, originalPlayer) {
